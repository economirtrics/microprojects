...
def loadImageAsDataFrame(fileName):
...
def run_test_case_2(red, green, blue):
  ...
  if isinstance(red, numbers.Number):
    print("\u2705 `r_median` is a number!")
  else:
    print(f"\N{CROSS MARK} `r_median` must be a number -- but yours is a {type(red)}.")
    return

  if red == 255:
    print("\u2705 `r_median` has the correct value!")
  else:
    print(f"\N{CROSS MARK} `r_median` is not the correct value.  (Did you use only the image subset of only the top row?)")
    return


  if isinstance(green, numbers.Number):
    print("\u2705 `g_median` is a number!")
  else:
    print(f"\N{CROSS MARK} `g_median` must be a number -- but yours is a {type(green)}.")
    return

  if green == 0:
    print("\u2705 `g_median` has the correct value!")
  else:
    print(f"\N{CROSS MARK} `g_median` is not the correct value.  (Did you use only the image subset of only the top row?)")
    return


  if isinstance(blue, numbers.Number):
    print("\u2705 `b_median` is a number!")
  else:
    print(f"\N{CROSS MARK} `b_median` must be a number -- but yours is a {type(blue)}.")
    return

  if blue == 255:
    print("\u2705 `b_median` has the correct value!")
  else:
    print(f"\N{CROSS MARK} `b_median` is not the correct value.  (Did you use only the image subset of only the top row?)")
    return
...
def createImageDataFrame(img, fileName = None):
  ...
  if (width > 400 or height > 400) and (fileName != "base.jpg"):
    print()
    print(f"⚠️ Your file {fileName} is large! ⚠️")
    print("- If this is being used as a tile image, your mosaic may run for a LONG time due to the size of your photo.")
    print("- View the notebook `shrink-images.ipynb` to pre-shrink the images into a new `tiles-small` folder if you need to speed up your program.")
    print()
